...
manifest schema, protocol version (`protocol_version`), and Python APIs may still
...
Initial reference implementation of the Skillward Protocol.

### Added
- Protocol spec (`spec/SPEC.md`) and manifest JSON Schema
  (`spec/skill-manifest.schema.json`), `protocol_version: "0.1"`.
...
- `skillward_common`: the visibility/ACL authorization check shared by every
  backend, and a factory (`skill_backend.py`) for adding new minimal
  backends in a few lines.
- Python orchestrator (`skillward/`): discovery client, checksum-verifying
  ...
  `StructuredTool` adapter (`skillward/langchain_tool.py`).
- Packaging (`pyproject.toml`) for `skillward` as an installable `skillward`
...
[Unreleased]: https://github.com/rkuma238/skillward/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/rkuma238/skillward/releases/tag/v0.1.0
